...
class ApproximateLocationMatcher extends BaseMatcher {
  ...
  compare(
    actual: ElementLocation,
    expected: ElementLocation,
    tolerance: number = 1
  ): CustomMatcherResult {
    const pass: boolean =
...
class ApproximateSizeMatcher extends BaseMatcher {
  ...
  compare(
    ...
    const message: string =
      `Expected {width: ${actual.width}, height: ${actual.height}}` +
      ` ${pass ? 'not ' : ''}to be within` +
      ` {width: ${expected.width - tolerance}..${expected.width + tolerance},` +
      ` height: ${expected.height - tolerance}..${
...
export const approximateGeometryMatchers: CustomMatcherFactories = {
  toBeApproximateLocation(util: MatchersUtil): CustomMatcher {
    const matcher: ApproximateLocationMatcher = new ApproximateLocationMatcher(
      util
    );
    return {
      compare: (
...
export interface ApproximateGeometryMatchersInterface<T>
  extends jasmine.Matchers<T> {
...
export const expect: (
  actual: any
) => ApproximateGeometryMatchersInterface<any> = <any>_global.expect;
