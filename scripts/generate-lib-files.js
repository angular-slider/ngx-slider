...
function generatePackageJson() {
  ...
  const libTemplateFile = path.resolve(
    __dirname,
    "../src/ngx-slider/package.json.template"
  );
  const libFile = path.resolve(__dirname, "../src/ngx-slider/package.json");

  const libTemplateConfig = JSON.parse(
    fs.readFileSync(libTemplateFile, { encoding: "utf8" })
  );
  const mainConfig = JSON.parse(
    fs.readFileSync(mainFile, { encoding: "utf8" })
  );
  ...
  for (let dependency of libTemplateConfig.dependenciesToCopyAsPeerDependenciesFromMainPackageJson) {
    libConfig.peerDependencies[dependency] =
      mainConfig.dependencies[dependency];
  }
...
function generatePublicApiTs() {
  const configFile = path.resolve(
    __dirname,
    "../src/ngx-slider/lib/public_api.json"
  );
