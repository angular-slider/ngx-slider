import { MathHelper } from "../../ngx-slider/lib/math-helper";

describe("MathHelper", () => {
  describe("roundToPrecisionLimit", () => {
    it("gets rid of digits to the left of precision limit", () => {
      expect(MathHelper.roundToPrecisionLimit(123456789010 + 1, 11)).toEqual(
        123456789010
      );
      expect(MathHelper.roundToPrecisionLimit(123456789010 - 1, 11)).toEqual(
        123456789010
    ...
    it("keeps digits to the right of precision limit", () => {
      expect(MathHelper.roundToPrecisionLimit(123456789010 + 10, 11)).toEqual(
        123456789010 + 10
      );
      expect(MathHelper.roundToPrecisionLimit(123456789010 - 10, 11)).toEqual(
        123456789010 - 10
  ...
  describe("isModuloWithinPrecisionLimit", () => {
    it("works for decimal modulo", () => {
      ...
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2 - 1e-12, 0.1, 10)
      ).toBeTruthy();
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2 + 1e-12, 0.1, 10)
      ).toBeTruthy();
      ...
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2, 0.1 - 1e-12, 10)
      ).toBeTruthy();
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2, 0.1 + 1e-12, 10)
      ).toBeTruthy();
      ...
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2 + 1e-9, 0.1, 10)
      ).toBeFalsy();
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2 - 1e-9, 0.1, 10)
      ).toBeFalsy();
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2, 0.1 - 1e-9, 10)
      ).toBeFalsy();
      expect(
        MathHelper.isModuloWithinPrecisionLimit(2, 0.1 + 1e-9, 10)
      ).toBeFalsy();
